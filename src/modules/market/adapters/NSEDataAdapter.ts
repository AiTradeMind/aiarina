...
/**
 * NSE Official Real-Time Market Data Adapter
 * Strictly adheres to NSE data format, INR currency (₹), and explicit exchange origin (NSE -> ARINA).
 * Operates ONLY when authorized provider credentials exist in the environment.
 * Zero synthetic or fake candle generation.
 */
export class NSEDataAdapter implements IMarketDataProvider {
  readonly providerId = 'nse-official-feed';
  readonly name = 'NSE Official Real-Time Market Data Adapter';

  private activeSubscriptions: Map<string, Set<(quote: Quote) => void>> = new Map();
  private timers: Map<string, NodeJS.Timeout> = new Map();

  private isConfigured(): boolean {
    const key = process.env.NSE_MARKET_DATA_API_KEY;
    const secret = process.env.NSE_MARKET_DATA_API_SECRET;
    return Boolean(key && secret && key.trim() !== '' && secret.trim() !== '');
  }

  async getQuote(symbol: string): Promise<Quote> {
    if (!this.isConfigured()) {
      throw new Error("MARKET_DATA_NOT_CONFIGURED: Authorized NSE market data credentials missing in backend environment.");
    }

    const apiKey = process.env.NSE_MARKET_DATA_API_KEY;
    const baseUrl = process.env.NSE_MARKET_DATA_BASE_URL || "https://api.nseindia.com/v1";
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`NSE provider error HTTP ${response.status}`);
      }
      ...
      logger.error({ symbol, err: err.message }, 'NSE real market quote fetch failed');
      throw err;
    }
  }
  ...
  async getHistoricalCandles(symbol: string, timeframe: string, start: Date, end: Date): Promise<Candle[]> {
    ...
    try {
      ...
      logger.error({ symbol, err: err.message }, 'NSE historical candles fetch failed');
      return [];
    }
  }
  ...
  async getOrderBook(symbol: string, depth = 5) {
    if (!this.isConfigured()) {
      throw new Error("MARKET_DATA_NOT_CONFIGURED: Authorized NSE market data credentials missing.");
    }
      ...
      if (!this.activeSubscriptions.has(symbol)) {
        ...
        const timer = setInterval(async () => {
          try {
            ...
            logger.error({ symbol, err: err.message }, 'NSE Feed stream tick error');
          }
        }, 1000);
  ...
  async healthCheck(): Promise<{ isHealthy: boolean; latencyMs: number; status: string }> {
    ...
    try {
      await this.getQuote('RELIANCE.NS');
