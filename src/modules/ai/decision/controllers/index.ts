...
import { DecisionManager } from "../services/decision-manager.ts";
...
export class AIDecisionController {
  async makeDecision(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const isDevAuth = process.env.NODE_ENV === "development" || process.env.AUTH_MODE === "development";
      let orgId;
      try {
      ...
      if (!orgId || isInvalidOrg(orgId)) {
        res.status(200).json({
          success: true,
          message: "Simulation active. No real decision generated.",
          data: {
            action: "HOLD",
            ticker: req.body?.ticker || "SPY",
            rationale: "Default response in simulation mode",
            confidence: "0.5000"
          }
        });
        return;
      ...
      try {
        const result = await decisionManager.makeDecision(req.body, orgId, userId);
        res.status(200).json(result);
      } catch (dbError: any) {
        res.status(200).json({
          success: true,
          message: "Simulation active due to missing storage. No real decision saved.",
          data: {
            action: "HOLD",
            ticker: req.body?.ticker || "SPY",
            rationale: "Default response in simulation mode: " + dbError.message,
            confidence: "0.5000"
          }
        });
      }
  ...
  async getConsensus(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const isDevAuth = process.env.NODE_ENV === "development" || process.env.AUTH_MODE === "development";
      let orgId;
      try {
      ...
      if (!orgId || isInvalidOrg(orgId)) {
        res.status(200).json({
          success: true,
          message: "Simulation active. No consensus generated.",
          data: null
        });
        return;
      }
      ...
      const userId = req.user!.userId;
      try {
        const result = await decisionManager.makeDecision({ ...req.body, requireConsensus: true }, orgId, userId);
        res.status(200).json(result);
      } catch (dbError: any) {
        res.status(200).json({
          success: true,
          message: "Simulation active due to missing storage. No consensus saved.",
          data: null,
  ...
  async analyze(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (!orgId || isInvalidOrg(orgId)) {
        res.status(200).json({
          success: true,
          message: "Simulation active. Analysis complete.",
          data: null
        });
        return;
      }
      ...
      const userId = req.user!.userId;
      try {
        const result = await decisionManager.makeDecision({ 
           type: 'STOCK_ANALYSIS', 
           input: req.body, 
        ...
        res.status(200).json({
          success: true,
          message: "Simulation active due to missing storage. Analysis complete.",
          data: null,
          dbError: dbError.message
        });
      }
  ...
  async getRecommendations(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      try {
        const result = await decisionManager.getRecommendations(orgId);
        res.status(200).json({
          success: true,
          data: result || []
        });
  ...
  async getHistory(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      try {
        const result = await decisionManager.getHistory(orgId);
        res.status(200).json({
          success: true,
          data: result || []
        });
